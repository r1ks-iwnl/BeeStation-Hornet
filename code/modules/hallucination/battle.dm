...
/datum/hallucination/battle/gun
	...
	var/fire_sound = 'sound/weapons/shotgunshot.ogg'
...
/datum/hallucination/battle/gun/proc/fire_loop(turf/source, shots_left = 3, hits = 0)
	...
	if(hits >= number_of_hits_to_end && prob(chance_to_fall))
		addtimer(CALLBACK(hallucinator, TYPE_PROC_REF(/mob/, playsound_local), source, "bodyfall", 25, TRUE), next_hit_sound)
		qdel(src)
...
/datum/hallucination/battle/stun_prod/start()
	...
	hallucinator.playsound_local(source, "bodyfall", 25, TRUE)
...
/datum/hallucination/battle/harm_baton/proc/harmbaton_loop(turf/source, hits_remaing = 5)
	...
	hallucinator.playsound_local(source, "swing_hit", 50, TRUE)
	hits_remaing--
	if(hits_remaing <= 0)
		qdel(src)
